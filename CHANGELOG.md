...
### Fixed
- Plan mode now passes `--exit-code` to `octorules plan`, so exit code 2
  correctly signals detected changes. Previously always exited 0, causing
  the "No changes detected" message even when changes existed.

## [1.8.4] - 2026-04-10

### Fixed
- `comment.sh` now validates that `GITHUB_REPOSITORY` is set, failing fast with
